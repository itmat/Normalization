...
while(my $line = <IN>){
	...
	foreach my $infile (@list){
	    ...
	    my $x = "$submit $request_memory_option$mem $jobname_option $jobname -o $logname_u.$index.out -e $logname_u.$index.err < $sh";
	    if ($hn_only eq "true"){
		$ssh->system($x) or
	...
	my @list = glob("$temp_prefix*");
	    ...
	    my $x = "$submit $request_memory_option$mem $jobname_option $jobname -o $logname_nu.$index.out -e $logname_nu.$index.err < $sh";
	    if ($hn_only eq "true"){
		$ssh->system($x) or
		    die "remote command failed: " . $ssh->error;
	    }
	...
	my $temp_prefix = "$filedir/$id.sam2genes_temp.";
	    ...
	    my $x = "$submit $request_memory_option$mem $jobname_option $jobname -o $logname.$index.out -e $logname.$index.err < $sh";
	    if ($hn_only eq "true"){
		$ssh->system($x) or
		    die "remote command failed: " . $ssh->error;
	    }
	...
	if ($stranded eq "true"){
		...
		my $x ="$submit $request_memory_option$mem $jobname_option $jobname -o $logname_a.$index.out -e $logname_a.$index.err < $sh";
		if ($hn_only eq "true"){
		    $ssh->system($x) or
			die "remote command failed: " . $ssh->error;
		}
