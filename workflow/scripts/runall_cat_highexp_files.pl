...
my $USAGE = "perl runall_cat_highexp_files.pl <sample dirs> <loc> [options]
...
use Cwd 'abs_path';
my $path = abs_path($0);
$path =~ s/\/runall_cat_highexp_files.pl//;
my $numargs = 0;
my $type = "";
my $stranded = "";
my $njobs = 200;
...
while(my $line = <IN>){
    chomp($line);
    my $id = $line;
    my $shfile = "$shdir/cat_highexp_files.$id.sh";
    my $jobname = "$study.cat_highexp_files";
    my $logname = "$logdir/cat_highexp_files.$id";
    open(OUTFILE, ">$shfile");
    print OUTFILE "perl $path/cat_highexp_files.pl $id $LOC $type $stranded\n";
    close(OUTFILE);
