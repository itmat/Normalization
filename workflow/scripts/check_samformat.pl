...
my $USAGE = "perl check_samformat.pl <aligned files> [options]
...
for my $aligned_file (@aligned_files) {
    ...
    while (!eof SAM) {
        ...
        if ($chr ne "*"){
            unless ($first =~ /HI:i:/){
                die "\nINPUT FORMAT ERROR: SAM file \"$aligned_file\" must have the HI tag.\n\n$first\n";
            }
            unless ($first =~ /(N|I)H:i:/){
                die "\nINPUT FORMAT ERROR: SAM file \"$aligned_file\" must have the NH or IH tag.\n\n$first\n";
            }
        }
