...
This will remove all rows from <sam infile> except those that satisfy all of the follo\
wing:
...
while(my $line = <INFILE>) {
    ...
    open(OUTFILE, ">$shfile");
    print OUTFILE "perl $path/filter_sam_gnorm.pl $LOC/$dir/$sam_name $LOC/$dir/$id.filtered.sam $idsfile $se $use_chr_names $use_mito_names\n";
    close(OUTFILE);
