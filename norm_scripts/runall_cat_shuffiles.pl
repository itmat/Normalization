...
my $USAGE = "perl runall_cat_shuffiles.pl <sample dirs> <loc> [options]
...
use Cwd 'abs_path';
my $path = abs_path($0);
$path =~ s/\/runall_cat_shuffiles.pl//;
my $numargs = 0;
my $type = "";
my $stranded = "";
my $njobs = 200;
...
while(my $line = <IN>){
    chomp($line);
    my $id = $line;
    my $shfile = "$shdir/a" . $id . "cat_shuffiles.$id.sh";
    my $jobname = "$study.cat_shuffiles";
    my $logname = "$logdir/cat_shuffiles.$id";
    open(OUTFILE, ">$shfile");
    print OUTFILE "perl $path/cat_shuffiles.pl $id $LOC $type $stranded -normdir $normdir\n";
    close(OUTFILE);
