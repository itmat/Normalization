...
if(@ARGV<7) {
    die "Usage: quantify_exons_filterhighexp.pl <exons file> <sam file> <output file> <exon sam out> <intron sam out> <loc> <highexp exons file>[options]
...
<highexp exons file> list of highly expressed exons. has one line per exon, each line is in the format chr:start-end
...
my %REMOVE;
open(HIGH, $highexp) or die "cannot find file '$highexp\n";
while($line = <HIGH>){
    chomp($line);
    if ($line =~ /([^:\t\s]+):(\d+)-(\d+)/) {
        $chr = $1;
...
while ($line1 = <INFILE>) {
	...
	if (($exon_sam_outfile ne "none") && ($print eq "true")){
	    if($forward_only eq 'true' && $reverse_only eq 'false') {
