...
use Cwd 'abs_path';
my $path = abs_path($0);
$path =~ s/\/runall_cat_genes_files.pl//;
my $numargs = 0;
my $type = "";
my $type_norm = "";
my $stranded = "";
...
while(my $line = <IN>){
    chomp($line);
    my $id = $line;
    my $shfile = "$shdir/a" . $id . "cat_genes_files.sh";
    my $jobname = "$study.cat_genes_files";
    my $logname = "$logdir/cat_genes.$index.$id";
    open(OUTFILE, ">$shfile");
    print OUTFILE "perl $path/cat_genes_files.pl $id $LOC $type $type_norm $stranded\n";
    close(OUTFILE);
